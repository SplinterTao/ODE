...
nearsimmelian=numeric(0)
simmelian=numeric(0)
idegreepopularity=numeric(0)
odegreepopularity=numeric(0)
istar2=numeric(0)
istar3=numeric(0)
ostar2=numeric(0)
ostar3=numeric(0)
...
for (i in 1:length(My_full_list)){
  stat=summary(My_full_list[[i]]~ttriple+ctriple+density+twopath+mutual+balance+cycle(4)+intransitive+nearsimmelian+simmelian+idegreepopularity+odegreepopularity+istar(2)+istar(3)+ostar(2)+ostar(3)+simmelianties)
  ttriple=c(ttriple,stat[1])
  ctriple=c(ctriple,stat[2])
  density=c(density,stat[3])
  twopath=c(twopath,stat[4])
  mutual=c(mutual,stat[5])
  balance=c(balance,stat[6])
  cycle4=c(cycle4,stat[7])
  intransitive=c(intransitive,stat[8])
  nearsimmelian=c(nearsimmelian,stat[9])
  simmelian=c(simmelian,stat[10])
  idegreepopularity=c(idegreepopularity,stat[11])
  odegreepopularity=c(odegreepopularity,stat[12])
  istar2=c(istar2,stat[13])
  istar3=c(istar3,stat[14])
  ostar2=c(ostar2,stat[15])
  ostar3=c(ostar3,stat[16])
  simmelianties=c(simmelianties,stat[17])
}

summary_statistic_matrix_original=data.frame(labels=labels)
summary_statistic_matrix_original["tcdiff"]=tcdiff
summary_statistic_matrix_original["twopath"]=twopath
summary_statistic_matrix_original["density"]=density
summary_statistic_matrix_original["mutual"]=mutual
summary_statistic_matrix_original["balance"]=balance
summary_statistic_matrix_original["ctri"]=ctri
summary_statistic_matrix_original["cycle4"]=cycle4
summary_statistic_matrix_original["idegreesquare"]=idegree_square
summary_statistic_matrix_original["odegreesquare"]=odegree_square
summary_statistic_matrix_original["diff_degree_pop"]=idegree_square-odegree_square
summary_statistic_matrix_original["sum_degree_pop"]=idegree_square+odegree_square
summary_statistic_matrix_original["intransitive"]=intransitive
summary_statistic_matrix_original["istar2"]=istar2
summary_statistic_matrix_original["istar3"]=istar3
summary_statistic_matrix_original["ostar2"]=ostar2
summary_statistic_matrix_original["ostar3"]=ostar3
summary_statistic_matrix_original["simmelianties"]=simmelianties
summary_statistic_matrix_original["nearsimmelian"]=nearsimmelian
summary_statistic_matrix_original["simmelian"]=simmelian

for (i in 1:50){
  sample=sample(1:nrow(summary_statistic_matrix_original),size=nrow(summary_statistic_matrix_original)*7/10)
  training=summary_statistic_matrix[sample,]
  validation=summary_statistic_matrix[-sample,]
  model=glm(labels~tcdiff+twopath+density+mutual+balance+ctri+cycle4+idegreesquare+odegreesquare+intransitive+istar2+istar3+ostar2+ostar3+simmelianties+nearsimmelian+simmelian,family="binomial",data=training)
